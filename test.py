...
class CircleCoverageSolver:
    ...
    def solve(self, coverage_weight=1.0, circle_weight=0.1):
        ...
        cell_vars = pulp.LpVariable.dicts(
            "cell", ((x, y) for x, y in area_cells), cat="Binary"
        )

        prob += coverage_weight * sum(
            cell_vars[x, y] for x, y in area_cells
        ...
        covered_area, coverage_percentage = self.calculate_exact_coverage(
            selected_circles
        )
    ...
    def visualize(self, circles, covered_cells, covered_area, coverage_percentage):
        ...
        for x, y in circles:
            circle = plt.Circle(
                (x, y), self.circle_radius, fill=False, color="blue", alpha=0.5
            )
            ax.add_patch(circle)
