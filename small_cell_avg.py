...
class GradientCircleCoverageSolver:
    ...
    def solve(self, min_light_level=0.2):
        ...
        minimum coverage in each small area cell.

        Args:
            min_light_level (float): Minimum light level required in each small area cell (default: 0.2)
        """
        ...
        for cell_x, cell_y in area_cells:
            ...
            # Apply minimum light level constraint to each small area cell
            prob += cell_vars[cell_x, cell_y] >= min_light_level
        ...
        min_coverage = min(cell_coverage.values()) if cell_coverage else 0
        max_coverage = max(cell_coverage.values()) if cell_coverage else 0
        avg_coverage = sum(cell_coverage.values()) / len(cell_coverage) if cell_coverage else 0
        
        print(f"\nCoverage Statistics:")
        print(f"Minimum cell coverage: {min_coverage:.3f}")
        print(f"Maximum cell coverage: {max_coverage:.3f}")
        print(f"Average cell coverage: {avg_coverage:.3f}")
    ...
    def visualize(self, circles, cell_coverage):
        ...
        metrics_text = (
            ...
            f"Min Coverage: {min(cell_coverage.values()):.2f}\n"
            f"Max Coverage: {max(cell_coverage.values()):.2f}\n"
            f"Avg Coverage: {sum(cell_coverage.values()) / len(cell_coverage):.2f}"
        )
